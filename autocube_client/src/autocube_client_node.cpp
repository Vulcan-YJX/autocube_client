...
: Node("autocube_client_node", options), running_(true)
{
  ...
  user_cmd_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(user_cmd_topic_, 10);
...
void AutocubeClientNode::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  twist_msg.set_linear_x(msg->twist.twist.linear.x);
  twist_msg.set_linear_y(msg->twist.twist.linear.y);
  twist_msg.set_linear_z(msg->twist.twist.linear.z);
  twist_msg.set_angular_x(msg->twist.twist.angular.x);
  twist_msg.set_angular_y(msg->twist.twist.angular.y);
  twist_msg.set_angular_z(msg->twist.twist.angular.z);
...
{
  auto battery_average = (battery1_percent + battery1_percent) / 2.0;
  autocube::BatteryMessage battery_msg;
  battery_msg.set_battery_one(battery1_percent);
  battery_msg.set_battery_two(battery1_percent);
